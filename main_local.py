...
from mongo_service import gpts_collection, create_new_gpt, update_gpt, delete_gpt, delete_gpts, delete_chat_history, fetch_chat_history, get_usecases, update_gpt_instruction, update_message
...
async def create_gpt(request: Request, loggedUser: str = Cookie(None), gpt: str = Body(...), files: list[UploadFile] = File(...)):
    try:
        ...
        if loggedUser != None and loggedUser != "N/A":
            ...
            if gpt.use_rag:
                file_upload_status = handle_upload_files(gpt, files)
